# Piano pagamenti 80 / 20

## Struttura

| Tranche | Quota | Importo (36 ospiti) | Evento scatenante | Condizione sospensiva |
|---|---|---|---|---|
| **1ª** | 80 % | **57.600 €** | Arrivo degli ospiti | Nessuna |
| **2ª** | 20 % | **14.400 €** | Partenza degli ospiti | **Controllo documentale superato** |

> Gli importi vanno ricalcolati sul numero **effettivo** di ospiti al momento dell'emissione.
> Riferimento per unità: 2.000 € netti/ospite → 1.600 € (80 %) + 400 € (20 %).

## Tranche 1 — 80 %

| Voce | Stato |
|---|---|
| Evento scatenante | Arrivo — **01/03/2027** (impostata, da confermare con la capofila) |
| Emissione fattura | **Qualche giorno prima dell'arrivo** (deciso 16/09; proposta: 24/02/2027) |
| Termine di pagamento | **Incasso il giorno dell'arrivo, 01/03/2027** (deciso 16/09; da scrivere nel contratto) 🟡 |
| Fattura emessa il | — |
| Numero fattura | — |
| Incassata il | — |

### Da fissare nel contratto

- [ ] "Arrivo" = arrivo del **primo** ospite o dell'**ultimo**? Se scaglionato, quale data vale?
- [x] Fattura emessa **prima** dell'arrivo, di qualche giorno (16/09)
- [x] Pagamento: bonifico con valuta il giorno dell'arrivo (16/09) — da mettere nel contratto con la data esatta
- [ ] Interessi di mora in caso di ritardo

> **Nota di cassa:** i costi di alloggio si sostengono in gran parte **prima o durante**
> l'arrivo, mentre l'80 % arriva il giorno dell'arrivo. Prima di quel giorno i soci anticipano
> in parti uguali, con fondi propri, e senza caparre agli immobili (scrittura privata, art. 5 e 10).

## Tranche 2 — 20 %

| Voce | Stato |
|---|---|
| Evento scatenante | Partenza — **30/05/2027** (derivata: 90 notti) |
| Condizione | Controllo documentale — **elenco non concordato** 🔴 |
| Termine di consegna documenti | `DA DEFINIRE` 🔴 |
| Termine di contestazione capofila | `DA DEFINIRE` 🔴 |
| Termine di pagamento | `DA DEFINIRE` 🔴 |
| Fattura emessa il | — |
| Numero fattura | — |
| Incassata il | — |

### Il rischio del 20 %

14.400 € sono **condizionati a una verifica fatta dalla controparte**. Senza paletti scritti,
la capofila può rinviare o rifiutare il saldo con motivazioni discrezionali, dopo che il
servizio è già stato interamente erogato e i costi già sostenuti.

**Presidi minimi** (dettaglio in [`../02-contratto/02-03-checklist-documentale.md`](../02-contratto/02-03-checklist-documentale.md)):

1. Elenco tassativo dei documenti, allegato al contratto.
2. Termine di decadenza per le contestazioni: scaduto, silenzio = accettazione.
3. Contestazione **parziale** → si paga la parte non contestata.
4. Diritto di **integrare** un documento mancante entro un termine breve.

## Tracciamento

Ogni fattura emessa e ogni incasso vanno registrati come riga nel foglio `Movimenti` del
registro su Google Fogli — link in [`movimenti-economici.md`](movimenti-economici.md).
