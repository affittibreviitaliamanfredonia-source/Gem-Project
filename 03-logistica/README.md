# 03 · Logistica

**Stato: 🟡 In definizione — 36 camere singole da confermare, date non fissate.**

| File | Contenuto |
|---|---|
| [`03-01-alloggi.md`](03-01-alloggi.md) | Strutture, camere singole, costi, stato prenotazioni |
| [`03-02-elenco-ospiti.md`](03-02-elenco-ospiti.md) | Registro dei 36 ospiti e assegnazione camere |
| [`03-04-regolamento-casa.md`](03-04-regolamento-casa.md) | Regolamento della casa, italiano e spagnolo, con deposito, silenzio, minorenni, danni; da firmare all'arrivo | 🟡 Bozza v0.1 |
| [`03-03-transfert-e-mobility.md`](03-03-transfert-e-mobility.md) | Trasporti da/per aeroporto e spostamenti locali |

## ⚠️ Regola operativa

**Nessuna prenotazione vincolante o caparra prima della firma del contratto.**
Ogni impegno assunto prima è rischio a carico esclusivo della parte italiana:
se il numero di ospiti cala o il progetto salta, il costo resta.

Se la capofila chiede di anticipare, la richiesta va messa a verbale in
[`../05-log-decisioni/`](../05-log-decisioni/) prima di procedere.

## Cosa manca per chiudere questa fase

- [x] Data di arrivo impostata: **01/03/2027** → partenza **30/05/2027** (90 notti) — *da confermare con la capofila*
- [ ] Numero definitivo di ospiti confermato per iscritto
- [ ] Preventivi alloggio ottenuti (almeno 2 strutture a confronto)
- [ ] Preventivi transfert ottenuti
- [ ] Soluzione mobility definita e quotata
- [ ] Costo variabile totale per ospite calcolato → sblocca il calcolo del margine
